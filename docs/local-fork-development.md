...
현재 **새 비공개 GHCR 패키지 초기 생성은 제공·검증되지 않았습니다.**
`MSA_RELEASE_ENABLED=false`, `MSA_PROMOTION_ENABLED=false`를 유지합니다.
Actions 변수와 `read:packages` 토큰만 준비하면 전체 환경이 완성되는 상태가 아닙니다.
...
처음부터 비공개라고 가정하지 않습니다. 별도 초기 생성·검증 절차는 아직 제공하지 않습니다.
...
확인해야 합니다. 현재 개인 포크의 새 비공개 패키지 초기 생성 및 GHCR→GitOps 전체 연결은
검증 완료로 표시하지 않습니다. 로컬 이미지로 실행한 Kubernetes 검증이나 통합 전 `GovBiz-Team`의
비공개 배포 기록을 개인 포크의 비공개 발행·pull 성공 증거로 대신하지 않습니다.
